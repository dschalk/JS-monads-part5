...
function main(sources) {
  ...
  return {
    ...
    code.arrayFuncs, 
      ...
      code.updateCalc,
      h('p', ' Note the line: '   ),  
      h('pre', ' .bnd(() => O.mM$1.bnd(push, O.mM7.x, mM$1) '  ),
      h('p', ' From the definition of "push", we see that the computed number in O.mM7 is pushed into the array which is held in O.mM$1, mM$1.ret() is called on the expanded array, and mM$1 is returned. From the definition of "ret" in Monad$, it is apparent that the expanded array that was the argument for mM$1.ret() gets pushed into mM$1.stream. That causes mM$1Action to add the array to O.mMhistorymM1.x and update the virtual DOM with the new numbers. '   ),
      h('h2', 'Some More Code Analysis' ),
      h('p', 'Information cycles into the "main" function, the function that returns the virtual DOM description, carrying a single argument: "sources". sources.WS provides websockets messages to the messages stream, which parses and acts upon them as follows: '  ),  
      code.messages,
      h('p', 'When a message prefixed by "CA#$42" arrives from the server, the following code is executed: '  ),  
      code.C42,
      h('p', ' As always, mM$1.ret() is called on array. In this case, it is the new roll of the dice. mM$1.ret() causes mM$1.stream to push the information to mM$1Action which updates virtual DOM number display and adds the array to O.mMhistorymM1.x ' ), 
      h('hr'),
      ...
      h('p', ), 
      h('hr',),  
      h('span.tao', 'The next installment in this series features a todo list that is shared within groups over websockets. It, along with prior installments, are available at '  ), 
