...
function main(sources) {
  ...
  return {
    ...
    code.arrayFuncs, 
      ...
      h('p', ' From the definition of "push", we see that the computed number in O.mM7 gets pushed into the array which is held in O.mM$1, mM$1.ret() gets called on the expanded array, and mM$1 gets returned. From the definition of "ret" in Monad$, it is apparent that the expanded array that was the argument for mM$1.ret() pushed in mM$1.stream. That causes mM$1Action to add the array to O.mMhistorymM1.x and update the virtual DOM with the new numbers. '   ),
